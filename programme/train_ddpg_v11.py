...
# --- 奖励函数权重配置 ---
# Reward = REWARD_STEP + PosReward - W_TORQUE * TorquePenalty
REWARD_STEP = 1.0       # [新增] 每步的基础生存奖励
REWARD_W_POS = 10.0     # [修改] 位置奖励权重 (因为改为了指数型，权重适当调整)
POS_ALPHA = 20.0        # [新增] 误差敏感度参数 (决定指数衰减的快慢)
REWARD_W_TORQUE = 0.5   # [修改] 二次方力矩惩罚权重 (由于二次方值较小，需放大权重)
...
# [修改] 模型保存在当前根目录 (用 "." 表示当前目录)
MODEL_DIR = "."
CHECKPOINT_FILE = "checkpoint.pth" # [新增] 断点续训状态文件
...
class ImpedanceEnv:
    ...
    def step(self, action):
        ...
        # [修改] 1. 非线性位置奖励 (指数型): 误差越小，奖励越接近 REWARD_W_POS
        pos_reward = REWARD_W_POS * np.exp(-POS_ALPHA * pos_err)
        
        # [修改] 2. 非线性力矩惩罚 (二次方): 容忍微小受力，严厉惩罚大受力
        tau_penalty = np.mean((np.abs(tau_l) / TORQUE_LIMITS)**2) + np.mean((np.abs(tau_r) / TORQUE_LIMITS)**2)
        
        # [修改] 3. 综合单步奖励
        reward = REWARD_STEP + pos_reward - REWARD_W_TORQUE * tau_penalty
...
def train():
    ...
    # [新增] 断点续训逻辑：尝试加载上次未完成的进度
    if os.path.exists(CHECKPOINT_FILE):
        print(f"\n>>> 发现断点文件 {CHECKPOINT_FILE}，正在恢复训练状态...")
        ...
        a_opt.load_state_dict(checkpoint['a_opt'])
        c_opt.load_state_dict(checkpoint['c_opt'])
        start_ep = checkpoint['episode'] + 1
        reward_history = checkpoint['reward_history']
        print(f">>> 成功恢复！将从第 {start_ep} 轮继续训练。")
    else:
        # 清空旧的结果文件
        if os.path.exists(RESULT_FILE): os.remove(RESULT_FILE)
        print(f"\n>>> 开始全新的训练 | Device: {DEVICE} | 目标: 优化 MBK")
    ...
    try:
        for ep in range(start_ep, 200):
            ...
            # [恢复] 每轮训练结束后，停止一秒 (同时修复了上一版的缩进)
            time.sleep(1.0)

    except KeyboardInterrupt:
        print("\n\n>>> 训练被手动停止")
    finally:
        # [新增] 训练结束或中断时，保存最终的模型参数到根目录
        print("\n>>> 正在保存最终模型到根目录 (final_actor.pth, final_critic.pth)...")
