...
# [新增] UR10 机械臂本体动力学参数
# 连杆质量 (kg) [Link1, Link2, Link3, Link4, Link5, Link6]
UR10_LINK_MASSES = [7.1, 12.7, 4.27, 2.0, 2.0, 0.365]

# 连杆质心位置 (CoM) 相对于连杆坐标系 [x, y, z] (简化估算值)
UR10_LINK_COMS = [
    [0, -0.02, 0],      # Link 1
    [0.3, 0, 0.1],      # Link 2 (大臂)
    [0.25, 0, 0],       # Link 3 (小臂)
    ...
    try:
        ...
        for step in range(TOTAL_STEPS + 1):
            ...
            if step == disturb_step:
                ...
                physics.pos += np.array([0.15, 0, 0]) # 或者直接位置突变
                is_disturbed = True
            ...
            # 计算总力矩 (重力矩 + 外部负载映射力矩)
            tau_l = robot_l.compute_total_torque(q_l, half_load)
            tau_r = robot_r.compute_total_torque(q_r, half_load)
